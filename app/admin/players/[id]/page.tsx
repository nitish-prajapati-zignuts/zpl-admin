 ...
 */
    ...
    return (
        <div className="w-full min-h-screen bg-[#f8fafc] p-4 lg:p-6 text-slate-900">

            {/* --- TEAM DASHBOARD ROW --- */}
            <div className="grid grid-cols-4 lg:grid-cols-8 gap-2 mb-4">
                {teams.map((team) => (
                    <div key={team._id} className="bg-white border border-slate-200 rounded-xl p-2 shadow-sm flex flex-col items-center text-center">
                        <p className="text-[14px] font-bold text-slate-800 truncate w-full mb-0.5">{team.name}</p>
                        <p className="text-[12px] font-black text-indigo-600 leading-none">
                            ₹{formatIndianCurrency(team.budgetRemaining)}
                        </p>
                        <div className="mt-1 flex items-center gap-1">
                            <div className="h-1 w-8 bg-slate-100 rounded-full overflow-hidden">
                                <div
                                    className={`h-full rounded-full ${team.players.length >= 12 ? 'bg-emerald-500' : 'bg-amber-400'}`}
                                    style={{ width: `${Math.min(100, (team.players.length / 12) * 100)}%` }}
                                />
                            </div>
                            <span className="text-[12px] font-bold text-slate-400">{team.players.length}/12</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                {/* --- HEADER SECTION --- */}
                <div className="relative overflow-hidden rounded-[1.5rem] bg-white border border-slate-200 shadow-sm p-4">
                    ...
                    <div className="relative flex flex-col md:flex-row justify-between gap-6 items-center md:items-start">
                        ...
                        <div className="relative group shrink-0">
                            <div className="size-20 lg:size-24 rounded-2xl overflow-hidden border-4 border-white shadow-xl bg-slate-100 flex items-center justify-center transition-all group-hover:scale-105">
                                {player.photoUrl ? (
                                    <img src={player.photoUrl} alt={player.name} className="size-full object-cover" />
                                ) : (
                                    <User size={32} weight="duotone" className="text-slate-300" />
                                )}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    <StatCard icon={<Target size={14} weight="duotone" />} label="Strategic Role" value={player.role || "All-Rounder"} color="bg-rose-50 text-rose-600" />
                    <StatCard icon={<GenderIntersex size={14} weight="duotone" />} label="Gender" value={player.gender.toUpperCase()} color="bg-blue-50 text-blue-600" />
                    <StatCard icon={<Wallet size={14} weight="duotone" />} label="Base Price" value={player.basePrice === 0 ? "Free Agent" : `₹${player.basePrice.toLocaleString()}L`} color="bg-amber-50 text-amber-600" />
                    <StatCard icon={<Tag size={14} weight="duotone" />} label="Sold Price" value={player.finalAmount ? `₹${player.finalAmount.toLocaleString()}L` : "Active"} color="bg-emerald-50 text-emerald-600" />
                    <StatCard icon={<ShieldCheck size={14} weight="duotone" />} label="Franchise" value={player.soldTo || "Unassigned"} color="bg-indigo-50 text-indigo-600" />
                    <StatCard icon={<GradientIcon size={14} weight="duotone" />} label="Grade" value={player.grade || "Unassigned"} color="bg-indigo-50 text-indigo-600" />
                </div>
            </div>
    ...
    return (
        <Card className="border-none shadow-sm hover:shadow-md transition-all rounded-2xl bg-white overflow-hidden group border border-transparent hover:border-slate-100">
            <CardContent className="p-2.5 flex flex-col gap-2">
                <div className={`size-7 flex items-center justify-center rounded-lg ${color} transition-transform group-hover:scale-110 shadow-sm`}>
                    {icon}
                </div>
                <div>
